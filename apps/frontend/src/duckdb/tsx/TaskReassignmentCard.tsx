...
const TaskReassignmentCard: React.FC<TaskReassignmentCardProps> = ({
  ...
  const handleDependenciesCleared = () => {
    // Recheck dependencies after clearing
    checkDependencies()
  }
  ...
  const handleReassign = async () => {
    ...
    try {
      ...
      const userCheck = await queryExecutor.executeQuery(`SELECT id FROM ddb.users WHERE id = ${selectedUserId}`)
      ...
      const query = `UPDATE ddb.tasks SET assigned_to = ${selectedUserId} WHERE id = ${task.id}`
      ...
      <div className="space-y-6">
        <div className="bg-white rounded-[20px] shadow-[0_2px_8px_rgba(0,0,0,0.08)] p-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-2xl font-bold text-[#fc8181]">Task Reassignment Blocked</h2>
          </div>
          ...
          <div className="bg-[#fff4e6] border-l-4 border-[#ffa940] p-4 rounded-[12px]">
            <div className="flex items-start gap-3">
              ...
              <p className="text-[#2d3748] text-sm">
                <span className="font-semibold">This task cannot be reassigned</span> due to existing dependencies in other tables. Please clear all dependencies first to proceed with reassignment.
              </p>
            </div>
          </div>
        </div>
